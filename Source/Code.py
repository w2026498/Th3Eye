...
if monitor_iface:
    iface = monitor_iface
    print(f"Detected monitor interface: {iface}")
else:
...
attack_names = {
    1: "Deauthentication", 2: "Beacon Flood", 3: "Probe Request Flood",
    4: "Auth/Assoc Flood", 5: "Evil Twin / Rogue AP", 6: "PMKID / EAPOL",
    7: "CTS/RTS Flood", 8: "TKIP MIC Failure", 9: "Null Function / Power-Save DoS",
    10: "ICMP/ARP Replay", 11: "Fragmentation Attack",
}

print(f"\nLearning baselines for: {', '.join(attack_names[s] for s in selected)}")

# ─────────────────────────────────────────────────────────────────────────────
# 3. SELECT SOURCE (PCAP or Live)
# ─────────────────────────────────────────────────────────────────────────────
print("\n=== Baseline Source ===")
...
probe_per_mac = defaultdict(int)
arp_per_mac = defaultdict(int)
frag_per_mac = defaultdict(int)
bssid_channels = defaultdict(set)
duration_minutes = 0
...
def analyse_packet(pkt):
    ...
    # (Same analysis logic as before - kept unchanged for brevity)
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. ASK FOR FILENAME + AUTO-GENERATE DETECTOR SCRIPT
# ─────────────────────────────────────────────────────────────────────────────

print("\n=== Save Generated Detector Script ===")
while True:
    filename = input("Enter the filename to save the detector as (e.g. my_detector.py): ").strip()
    if filename.endswith('.py'):
        break
    else:
        print("Filename should end with .py. Please try again.")

# Build threshold dictionary string
thresh_dict_lines = []
for attack_id in selected:
...
def monitor_loop():
    while True:
        ...
        def analyse(pkt):
            ...
            if 1 in SELECTED and ptype == 0 and subtype == 12: counts[1] += 1
            if 2 in SELECTED and ptype == 0 and subtype == 8: counts[2] += 1
            if 3 in SELECTED and ptype == 0 and subtype == 4:
                counts[3] += 1
                probe_mac[src_mac] += 1
            if 4 in SELECTED and ptype == 0 and subtype in (0, 2, 11): counts[4] += 1
            if 5 in SELECTED and ptype == 0 and subtype == 8 and bssid:
                ch = None
                if pkt.haslayer(RadioTap):
                    ch = getattr(pkt[RadioTap], "Channel", None)
                if ch:
                    ...
                    if len(bssid_chans[bssid]) > 1:
                        counts[5] += 1
            if 6 in SELECTED and pkt.haslayer(EAPOL): counts[6] += 1
            if 7 in SELECTED and ptype == 1 and subtype in (11, 12): counts[7] += 1
            if 8 in SELECTED and ptype == 2:
                fc = dot11.FCfield if hasattr(dot11, "FCfield") else 0
                if fc & 0x40: counts[8] += 1
            if 9 in SELECTED and ptype == 2 and subtype in (4, 12): counts[9] += 1
            if 10 in SELECTED and pkt.haslayer(ARP):
                counts[10] += 1
                arp_mac[src_mac] += 1
            if 11 in SELECTED and ptype == 2:
...
# Save the generated detector with user-specified name
with open(filename, "w") as f:
    f.write(new_code)

print(f"\n✅ Detector script '{filename}' created successfully!")
print("To start real-time protection, run:")
print(f" sudo python3 {filename}")
print("\nYou can re-run this learner with a different pcap to update the baseline.")
print("=" * 80)
